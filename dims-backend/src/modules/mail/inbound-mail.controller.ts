...
export class InboundMailController {
  ...
  @HttpCode(200)
  @ApiOperation({
    summary: "Inbound email webhook — called by Postfix/MTA with raw RFC 2822 email",
  })
  async receive(
    ...
    const expectedSecret = this.configService.get<string>("INBOUND_WEBHOOK_SECRET");
    ...
    try {
      ...
      this.logger.error(`Failed to process inbound email: ${(err as Error).message}`);
      throw err;
    }
  }
}
